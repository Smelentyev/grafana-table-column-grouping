...
const alphaColor = (color: string, opacity: number): string => {
  const result = colorManipulator.alpha(color, opacity) as unknown as { toRgbString?: () => string; toString?: () => string } | string;
  return typeof result === 'string' ? result : result.toRgbString?.() ?? result.toString?.() ?? String(result);
};
...
export const getGridStyles = (theme: GrafanaTheme2, enablePagination?: boolean, transparent?: boolean) => {
  ...
  const scrollbarThumb = alphaColor(theme.colors.text.primary, theme.isDark ? 0.35 : 0.25);
  const scrollbarTrack = alphaColor(theme.colors.text.primary, theme.isDark ? 0.15 : 0.1);
  ...
  return {
    ...
    menuItem: css({ maxWidth: theme.spacing(25) }),
...
export const getCellActionStyles = (theme: GrafanaTheme2, textAlign: TextAlign) =>
  css({
    ...
    background: alphaColor(theme.isDark ? theme.colors.background.primary : theme.colors.background.canvas, 0.7),
