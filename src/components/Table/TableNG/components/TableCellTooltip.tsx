import React, { CSSProperties, ReactElement, useMemo, useState, useRef, useEffect, memo, RefObject, useCallback } from 'react';
...
export const TableCellTooltip = memo(
  ({
    ...
    const tooltipCaretRef = useRef<HTMLDivElement | null>(null);
    ...
    useEffect(() => {
      if (pinned) {
        ...
        const currentCaret = tooltipCaretRef.current;

        const windowListener = (ev: Event) => {
          if (ev.target === currentCaret) {
            return;
          }
    ...
    const tooltipCaretCallback = useCallback((node: HTMLDivElement | null) => {
      (tooltipCaretRef as React.MutableRefObject<HTMLDivElement | null>).current = node;
      if (node) {
        setCellElement(node.closest<HTMLElement>('.rdg-cell'));
      } else {
        setCellElement(null);
      }
    }, []);
      ...
      <>
        {cellElement && (
          <Popover
            ...
            data-testid="table-tooltip-wrapper"
          />
        )}
        ...
        <div
          ...
          ref={tooltipCaretCallback}
          data-testid="table-tooltip-caret"
          aria-pressed={pinned}
