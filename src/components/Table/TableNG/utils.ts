...
// Local implementation of getTextColorForBackground
export function getTextColorForBackground(color: string): string {
  const c = tinycolor(color);
  return c.isDark() ? 'rgba(255, 255, 255, 0.9)' : 'rgba(0, 0, 0, 0.9)';
}

// Local implementation of getTextColorForAlphaBackground
export function getTextColorForAlphaBackground(color: string, isDark: boolean): string {
  const c = tinycolor(color);
